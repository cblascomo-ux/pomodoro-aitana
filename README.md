...
<head>
  <meta charset="UTF-8" />
  <title>Pomodoro Pop Aitana 🎧⏰</title>
...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-500 via-purple-500 to-blue-500 text-white">
  ...
  >
    ...
    <header class="mb-6">
      ...
      <p class="mt-2 text-sm sm:text-base text-pink-50">
        Modo estudio con vibes pop: tú, tu temario y Aitana en tu cabeza 🎤💜
      </p>
    </header>
    ...
    <div class="mb-6">
      ...
      <p class="mt-1 text-xs sm:text-sm text-pink-50">
        Concéntrate a tope durante la fase de trabajo y luego disfruta del descanso 😌
      </p>
    </div>
    ...
    <div class="mb-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-sm">
      <p id="pomodoroCount" class="font-semibold">
        🍅 Pomodoros completados: <span class="font-bold">0</span>/4
      </p>
      <p class="text-pink-50 text-xs sm:text-sm">
        Tras 4 pomodoros, tendrás un descanso largo 😴
      </p>
    </div>

    <!-- Notificación visual / mensaje -->
    <div
      id="notification"
      class="mb-4 text-xs sm:text-sm text-center bg-white/10 rounded-2xl px-3 py-2 border border-white/10 min-h-[2.5rem]"
      aria-live="assertive"
    >
      Lista para empezar. Cuando termine cada fase verás un aviso aquí 💫
    </div>
    ...
    >
      💖 “Modo Aitana ON: hoy vas a brillar estudiando.”
    </div>
    ...
    <section class="mt-4 text-left text-xs sm:text-sm bg-black/10 rounded-2xl px-4 py-3">
      <h2 class="font-semibold mb-2 flex items-center gap-1">
        📚 Cómo usar este Pomodoro
      </h2>
      <ol class="list-decimal list-inside space-y-1">
        <li>Guarda este código como <code>pomodoro-aitana.html</code>.</li>
        <li>Haz doble clic en el archivo para abrirlo en tu navegador.</li>
        <li>Pulsa <span class="font-semibold">“Iniciar”</span> para comenzar 25 min de trabajo.</li>
        <li>Al terminar, sonará un aviso y tendrás 5 min de descanso.</li>
        <li>Cada 4 pomodoros, el descanso será más largo para resetear mente y energía.</li>
      </ol>
    </section>
  </div>
  ...
  <script>
    ...
    const quotes = {
      work: [
        "🎧 “Ella me dice que quiere divertirse, yo le digo que toca estudiar un ratito.”",
        "🎶 “Vas a quedaaaaaaaaaarte (estudiando un ratito más) 🎧📚”",
        "🔥 “Y es que si esto está mal, que me lleven presa (el tema de los presos es otro 😜)”",
        "✨ “A por el pomodoro: tú eres una diosa de mujer. Una chula, una diabla de las que te lían.”"
      ],
      short: [
        "😌 “Respira, estira y una pausita o unos besitos a Rogelia y a seguir.”",
        "🌈 “Mini break: aguita, respirito, un wasapito a la mejor novia del mundo y a seguir.”",
        "🧃 “Un sorbo, un suspiro, un beso y una flor como diría Nino Bravo y vuelta al foco.”"
      ],
      long: [
        "🛋️ “Descanso largo oe oeee oeoeee: te lo has ganado.”",
        "🌙 “Toca descansito un poco más largooooooo.”",
        "🫶 “DESCANSOOOOOOO, ahora a buscar recetas en TikTok para cocinarle a Cris.”"
      ]
    };

    // Índices para avanzar en orden y sin repetir hasta completar el ciclo de cada modo
    ...
    function updatePhaseLabel() {
      if (currentMode === "work") {
        ...
        phaseLabel.textContent = "💆‍♀️ Descanso corto";
      } else {
        phaseLabel.textContent = "🌙 Descanso largo";
      }
    }
    ...
    function handlePhaseEnd() {
      ...
      if (currentMode === "work") {
        ...
        if (completedPomodoros % 4 === 0) {
          setNotification("✨ Fin de bloque de trabajo. ¡Descanso largo merecido!");
          setMode("long");
        } else {
          setNotification("🎉 ¡Buen trabajo! Toca descanso corto.");
          setMode("short");
        }
      } else if (currentMode === "short") {
        setNotification("🔥 Descanso corto terminado. Volvemos al modo foco.");
        setMode("work");
      } else {
        setNotification("💜 Fin del descanso largo. Nuevo ciclo de 4 pomodoros.");
        completedPomodoros = 0;
        updatePomodoroCount();
        setMode("work");
      }

      // Inicia automáticamente la siguiente fase
      startTimer();
    }
    ...
    function startTimer() {
      ...
      setNotification("⏱️ Temporizador en marcha. ¡A tope con esta fase!");
    ...
    function pauseTimer() {
      if (!isRunning) return;
      clearTimer();
      setNotification("⏸️ Temporizador en pausa. Respira y decide tu siguiente paso.");
    }
    ...
    function resetTimer() {
      ...
      updateTimerDisplay();
      setNotification("🔄 Todo reseteado. Lista para un nuevo ciclo Pomodoro.");
    }
